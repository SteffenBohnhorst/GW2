...
#define S_FUNCTION_NAME  bahnplaner_v1
...
#define MAXX 10
#define MAXY 10
#define MAX_LIST_ELEMENTS MAXX*MAXY
#define SIZEDELTA1 4
#define SIZEDELTA2 2
#define COST 1
...
void astern(int start[], int goal[]);
...
void astern(int start[], int goal[]) {
	...
	search_t NewCell = { 0,0,0,0,FALSE,FALSE };
	int Found = FALSE;
	int Resign = FALSE;
	...
	while (Found == FALSE || Resign == FALSE) {
		...
		for (i = 0; i < SIZEDELTA1; i++) {
			...
			else
			{
				...
				NewCell = Grid[expandNode.currX - 1 + delta[i][0]][expandNode.currY - 1 + delta[i][1]];
				if (NewCell.isChecked != TRUE && NewCell.isEmpty != TRUE) {
					element1_p = &Grid[NewCell.currX - 1][NewCell.currY - 1];
					...
					ActionTaken[NewCell.currX - 1][NewCell.currY - 1] = i + 1;
				}
			}
			if (NewCell.currX == goalPos_p->currX && NewCell.currY == goalPos_p->currY && NewCell.isEmpty != TRUE) {
				Found = TRUE;
				Resign = TRUE;
				element1_p->isChecked = TRUE;
#ifdef PRINTGRID
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (TRUE || !data->runonce){
        data->start[0]=1;
        data->start[1]=1;
        data->goal[0]=1;
        data->goal[1]=10;
        
    astern(data->start,data->goal);
    data->runonce=1;
    }
    y[0] = data->start[0];
    y[1] = data->start[1];
